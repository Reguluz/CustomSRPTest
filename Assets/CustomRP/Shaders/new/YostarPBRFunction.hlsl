#ifndef YOSTAR_PBR_INCLUDED
#define YOSTAR_PBR_INCLUDED
...
inline void YostarInitializeBRDFData(half3 albedo, half metallic, half3 specular, half smoothness, half alpha, out BRDFData outBRDFData)
{
...
half3 GetImageBasedReflectionLighting(half Roughness, half IndirectIrradiance, half3 reflectVector)
{
// #if HQ_REFLECTIONS
...
// #else
	...
	half3 SpecularIBL;
    //此处需替换为反射探针捕捉的CubeMap
    half3 SpecularIBLSample = DecodeHDREnvironment(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflectVector, AbsoluteSpecularMip), unity_SpecCube0_HDR);
...
half4 YostarFragmentPBR(YostarInputData inputData, half3 albedo, half metallic, half3 specular,
    half smoothness, half occlusion, half3 emission, half alpha)
{
    // BRDFData brdfData;
    // YostarInitializeBRDFData(albedo, metallic, specular, smoothness, alpha, brdfData);

    //**********Init Diffuse and Specular color
    half DielectricSpecular = 0.08 * specular;
	half3 DiffuseColor = albedo - albedo * metallic;	// 1 mad
    ...
    Color += FakeUE4Lightmap.rgb * DiffuseColor;//UE4 * View.IndirectLightingColorScale;
    ...
    half3 DiffuseGI = inputData.SH;
    IndirectIrradiance = Luminance(DiffuseGI);
    Color += DiffuseColor * DiffuseGI;//UE4 * View.IndirectLightingColorScale;
#endif
...
// MobileDirectionalLight.DirectionalLightDistanceFadeMADAndSpecularScale.z saves SpecularScale for direction light.
    Color += (Shadow * NoL) * mainLight.color * (DiffuseColor + SpecularColor /*UE4 * MobileDirectionalLight.DirectionalLightDistanceFadeMADAndSpecularScale.z*/ * CalcSpecular(Roughness, RoL, NoH, H, inputData.normalWS));
    // Environment map has been prenormalized, scale by lightmap luminance
    half3 SpecularIBL = GetImageBasedReflectionLighting(Roughness, IndirectIrradiance, R);
    ...
    // SpecularIBL *= surfaceReduction * lerp(SpecularColor, grazingTerm.xxx, fresnelTerm);
...
//     half3 color = GlobalIllumination(brdfData, inputData.bakedGI, occlusion, inputData.normalWS, inputData.viewDirectionWS);   ---->EnvBRDF
